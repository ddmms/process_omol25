...
def test_verify_processed_omol25_success(tmp_path):
    ...
    df = pd.DataFrame([
        {"geom_sha1": "hash_valid1", "test_energy": -50.123},
        {"geom_sha1": "hash_valid2", "test_energy": -60.456, "process_time_s": 1.23}
    ])
...
def test_verify_processed_omol25_structural_mismatch(tmp_path):
    ...
    df = pd.DataFrame([
        {"geom_sha1": "hash_miss1", "test_energy": -50.0},
    ...
    res = subprocess.run(cmd, capture_output=True, text=True)
    assert res.returncode == 1, "Verification falsely passed structural mismatch"
...
def test_verify_processed_omol25_property_mismatch(tmp_path):
    ...
    df = pd.DataFrame([
        {"geom_sha1": "hash_prop_err", "dipole": 0.123456789}
    ])
    df.to_parquet(pq_file)
    ...
    res = subprocess.run(cmd, capture_output=True, text=True)
    assert res.returncode == 1, "Verification falsely passed property mismatch"
