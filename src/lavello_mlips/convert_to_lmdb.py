...
def cv_xyz_to_lmdb(input_files: List[str], output_file: str):
    ...
    if path.suffix != ".aselmdb":
        ...
        logger.warning(f"Changing output extension to .aselmdb for Fairchem compatibility: {output_file}")
...
def main():
    parser = argparse.ArgumentParser(description="Convert between Extended XYZ and ASE-style LMDB database.")
    ...
    tolmdb.add_argument("-o", "--output", required=True, help="Output LMDB file (.aselmdb)")
    ...
    fromlmdb.add_argument("-o", "--output", required=True, help="Output Extended XYZ file")
