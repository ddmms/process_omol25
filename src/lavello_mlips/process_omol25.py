...
def parse_eigens(txt: str) -> Optional[Dict[str, Any]]:
    if not txt:
        return None
    # ⚡ Bolt: Fast-path string literal check before splitting large text blocks
    # ORCA output uses exact casing for these column headers. Bypassing splitlines()
    # and the regex loop if the required text is completely absent provides a massive speedup.
    if "OCC" not in txt and "occ" not in txt:
        return None
    ...
    i = 0
    while i < len(lines):
        # ⚡ Bolt: Fast-path substring check before invoking regex engine per line
        if ("OCC" in lines[i] or "occ" in lines[i]) and RE_COLS.search(lines[i]):
            tag = "R"
            back = (
