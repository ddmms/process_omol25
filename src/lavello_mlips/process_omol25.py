...
def geom_sha1(elems, coords, ndp: int = 6) -> Optional[str]:
    """Calculate SHA1 hash of geometry using a single string join for performance."""
    h = hashlib.sha1()
    s = "".join(
...
def parse_eigens(txt: str) -> Optional[Dict[str, Any]]:
    ...
    while i < len(lines):
        line = lines[i]
        # Fast path check before executing regex
        if ("E(E" in line or "e(e" in line) and RE_COLS.search(line):
            tag = "R"
            back = (
