...
def cog(coords):
    # ⚡ Bolt: Vectorize Center of Geometry calculation (O(N) numpy loop vs pure python)
    return np.mean(coords, axis=0).tolist()


def cnc(Z, coords):
    # ⚡ Bolt: Vectorize Center of Nuclear Charge calculation (O(N) numpy loop vs pure python)
