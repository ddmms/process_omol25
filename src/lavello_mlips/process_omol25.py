...
def cog(coords):
    # Bolt optimization: Vectorized calculation of center of geometry instead of pure Python loops
    return np.mean(np.asarray(coords), axis=0).tolist()


def cnc(Z, coords):
    # Bolt optimization: Vectorized calculation of center of nuclear charge instead of pure Python loops
    return np.average(np.asarray(coords), weights=np.asarray(Z), axis=0).tolist()
