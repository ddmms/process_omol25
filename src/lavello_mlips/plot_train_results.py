...
def parse_arguments():
    ...
    parser.add_argument(
        "--yaml", "-y", default=None, help="yaml file containinng all input and outut training data, works only with params in yaml file"
    ),
    parser.add_argument(
        "--device", "-d", default="cpu",  help="device on which to run mlip calculations"
    )
    parser.add_argument(
        "--arch", "-a", default="mace_mp",  help="device on which to run mlip calculations"
    )
    parser.add_argument(
    ...
    parser.add_argument("--log", "-l", default='plot.log', help="Custom title for the plot.")
    parser.add_argument(
        "--iso", "-i", default="isolated.xyz", help="file, containing isolated energies."
    )
    parser.add_argument(
        ...
        for head in config['heads']:
                ...
                if head == "pt_head":
                    ...
                    try:
                        ...
                        all_files['valid'] = _valid if isinstance(_valid,list) else [_valid]
                    except: 
                        pass 
                ...
                all_frames = list(chain(all_res['train'],all_res['test'],all_res['valid']))
                extract_and_plot(
                    xyz_path = all_frames,
                    e0s_path = output/iso_res ,
